  ...
  >
    <div class="relative w-full max-w-3xl md:max-h-full">
      ...
      <div class="sm:relative p-4 bg-white rounded-lg shadow sm:p-5">
        ...
        <!-- Modal body -->
        <form action="#" @submit.prevent="onSubmit" v-if="data.blog != null">
...
onUpdated(() => {
  if (currentId.value != data.blog._id) {
    currentId.value = data.blog._id;
    setFieldValue("title", data.blog.title);
    setFieldValue("content", data.blog.content);
  }
});
...
const onSubmit = handleSubmit(async (values) => {
  try {
    clearNuxtData();
    await data
      .editBlog(image.value, currentId.value, values.title, values.content)
      .then(() => {
